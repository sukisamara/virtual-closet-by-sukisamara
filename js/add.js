...
let step = 1;
let previewSrc = null;
let form = { name:'', brand:'', cost:'', category:'tops', subcategory:'', aesthetics:[], occasions:[], color:PALETTE[0], image:null };

function setForm(key,val) { form[key]=val; renderStep(); }
function toggleTag(arr,val) {
  const idx=form[arr].indexOf(val);
  if(idx>-1) form[arr].splice(idx,1); else form[arr].push(val);
  renderStep();
}
function handleFileInput(input) {
  const file=input.files[0]; if(!file) return;
  const r=new FileReader(); r.onload=e=>{ previewSrc=e.target.result; form.image=e.target.result; renderStep(); }; r.readAsDataURL(file);
}
function handleDrop(e) {
  e.preventDefault(); document.getElementById('uploadZone').classList.remove('drag');
  const file=e.dataTransfer.files[0]; if(!file||!file.type.startsWith('image/')) return;
  const r=new FileReader(); r.onload=ev=>{ previewSrc=ev.target.result; form.image=ev.target.result; renderStep(); }; r.readAsDataURL(file);
}
...
function submitForm() {
  ...
  items.unshift({ ...form, id:'item-'+Date.now(), cost:parseFloat(form.cost), wears:0, addedAt:Date.now() });
  saveItems(items);
  document.getElementById('main').innerHTML=`
  <div class="success-screen fade-up">
    <span class="success-star">✦</span>
    <h2>Added to your closet!</h2>
    <p class="t-adult" style="font-size:18px;color:var(--text-muted)">Taking you back…</p>
  </div>`;
...
function renderLeft() {
  ...
  <div class="add-left">
    <div class="upload-zone${previewSrc?' has-img':''}" id="uploadZone"
      ...
      ${previewSrc
        ...
        : `<div class="upload-empty">
            ...
            <p class="upload-note">PNG with transparent background works best</p>
          </div>`}
    ...
    <div class="color-section">
      ...
      <div class="palette">${PALETTE.map(c=>`<button class="swatch${form.color===c?' active':''}" style="background:${c}" onclick="setColor('${c}')"></button>`).join('')}</div>
    </div>
  </div>`;
}
...
function renderStep() {
  ...
  <div class="steps">
    ${['Photo & Details','Tags','Confirm'].map((lbl,i)=>{
      const n=i+1, cls=step===n?'active':step>n?'done':'';
      ...
      <div class="field-row">
        <div class="field-group"><label class="field-label">Category</label>
          <select class="field-select" onchange="form.category=this.value;form.subcategory='';renderStep()">
            ${CATEGORIES.map(c=>`<option value="${c.id}"${form.category===c.id?' selected':''}>${c.label}</option>`).join('')}
          </select></div>
        <div class="field-group"><label class="field-label">Specific Type</label>
  ...
  if(step===2) {
    ...
    <div class="form-step fade-up">
      <div class="tag-section">
        <label class="field-label">Aesthetic Tags <span class="optional">(pick all that apply)</span></label>
        <div class="tag-cloud">${AESTHETICS.map(a=>`<button class="pill${form.aesthetics.includes(a)?' active':''}" onclick="toggleTag('aesthetics','${a.replace(/'/g,"\\'")}')">${a}</button>`).join('')}</div>
      </div>
      <div class="tag-section">
        <label class="field-label">Occasion Tags <span class="optional">(when would you wear this?)</span></label>
        <div class="tag-cloud">${OCCASIONS.map(o=>`<button class="pill${form.occasions.includes(o)?' active':''}" onclick="toggleTag('occasions','${o.replace(/'/g,"\\'")}')">${o}</button>`).join('')}</div>
      </div>
    ...
    <div class="form-step fade-up">
      ...
      <div class="confirm-card">
        <div class="confirm-img" style="background:linear-gradient(135deg,${form.color}44,${form.color}99)">
          ${form.image?`<img src="${form.image}" alt="Preview" />`:`<span style="font-size:18px;color:var(--text-muted);padding:10px;text-align:center">${activeCat2?.label||''}</span>`}
        </div>
        <div>
  ...
  document.getElementById('main').innerHTML=`
  <div class="add-hdr fade-up"><span class="eyebrow">Grow your wardrobe</span><h1 class="section-title">Add New Item</h1></div>
  ${stepsHTML}
  <div class="add-layout">${renderLeft()}<div class="add-right">${stepContent}</div></div>`;
}
